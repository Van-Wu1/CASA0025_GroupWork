...
function getGlacierElevation(year) {
  var assetPath = 'users/ixizroiesxi/glacier/glacier_changes_' + year + '_3band';
  return ee.Image(assetPath).clip(defaultRegion);
}
// ===== [XinyiZeng] End =====
...
function getLayer(type, year) {
  if (type === 'Glacier') {
    var glacImg = getGLACImageByYear(year);
    return glacImg.visualize({ min: -6, max: 26, palette: PALETTE_GLACIER });
  } else if (type === 'NDVI') {
